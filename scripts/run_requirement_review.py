...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build deterministic requirement review cases from parsed DOCX main_body blocks")
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Path to output JSON. Defaults to data/output/<input_name>.requirement_review.json",
    )
    ...
    output_path = (
        Path(args.output)
        if args.output
        else Path("data/output") / f"{input_path.stem}.requirement_review.json"
    )
    ...
    payload = {
        "review_cases": [case.model_dump(mode="json") for case in review_cases],
        "review_decisions": [decision.model_dump(mode="json") for decision in review_decisions],
    }
    with output_path.open("w", encoding="utf-8") as file:
        json.dump(payload, file, ensure_ascii=False, indent=2)

    print(f"Done. Requirement review payload written to: {output_path}")
