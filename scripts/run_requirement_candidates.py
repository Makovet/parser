...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build requirement candidates from parsed DOCX main_body blocks")
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Path to output JSON. Defaults to data/output/<input_name>.requirement_candidates.json",
    )
    ...
    output_path = (
        Path(args.output)
        if args.output
        else Path("data/output") / f"{input_path.stem}.requirement_candidates.json"
    )
    ...
    print(f"Done. Requirement candidates written to: {output_path}")
