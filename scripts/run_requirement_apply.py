...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build applied requirement output from parsed DOCX main_body blocks")
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Path to output JSON. Defaults to data/output/<input_name>.requirement_apply.json",
    )
    ...
    output_path = (
        Path(args.output)
        if args.output
        else Path("data/output") / f"{input_path.stem}.requirement_apply.json"
    )
    ...
    payload = {
        "applied_requirements": [record.model_dump(mode="json") for record in applied_records],
        "apply_report": apply_report.model_dump(mode="json"),
    }
    with output_path.open("w", encoding="utf-8") as file:
        json.dump(payload, file, ensure_ascii=False, indent=2)

    print(f"Done. Requirement apply payload written to: {output_path}")
